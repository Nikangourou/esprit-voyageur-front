...
export default function Chat() {
  ...
  const altGenerateAllImages = () => {
    ...
    const tl = gsap
      ...
      .call(async () => {
        const trueContent =
          "The snowy slopes of Switzerland where a young student on their first skiing expedition accidentally collides with an elderly lady on a challenging red icy slope. The young skier, gliding uncontrollably with a look of panic, crashes into the elderly lady, causing a commotion. Surrounding skiers stare and murmur disapprovingly, while a ski patrol rushes to help the lady. The cold air is tense with the skier standing alone, looking embarrassed and isolated, contemplating the steep learning curve of both life and skiing. The backdrop should be a picturesque, snowy landscape with skiers in colorful gear and distant snow-capped peaks under a clear, bright sky.";
        const falseContent =
         "on the ski slopes of Val Cenis where a group of friends is gathered around a speed radar on a steep slope. The group is taking turns trying to achieve the highest speed, with a mix of excitement and competitive spirit. Show one skier, representing the narrator, descending the slope with a look of determination, while friends watch from the bottom, displaying shocked reactions. The atmosphere should be filled with crisp mountain air, bright sunlight reflecting off the snow, and a sense of camaraderie and fun. The picturesque mountain landscape and the colorful ski gear add to the vibrancy of this memorable day.";
        await generateImg(
          apiUrl,
          trueContent,
          gameId,
